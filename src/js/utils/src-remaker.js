...
const createTranslation = (imagesDataArray) => {
  const outputPromise = Promise.all(
    imagesDataArray.map(async (el) => {
      const result = { ...el };
      const translationObj = await translate(`${el.author}: ${el.name}`, { from: 'ru', to: 'en' });
      const translationSplittedArray = translationObj.text.split(': ');
      const translationAuthor = translationSplittedArray[0];
      const translationName = capitalizeFirstLetter(translationSplittedArray[1]);
